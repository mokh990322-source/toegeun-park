/* ============================================================
   ...
   ── 떨어지는 것과 가시는 다르다 ─────────────────────────
   떨어지면 그 사람만 자기 시작점에서 되살아난다. 발을 헛디딘 건 개인의
   실수라 개인이 물린다.
   가시에 닿거나 누가 "다시 하기"를 누르면 전원이 라운드 처음부터. 그건
   판이 깔아 둔 함정이라 팀이 같이 물려야 방해 요소가 무게를 갖는다.
   둘을 같이 묶으면 계단에서 한 번 헛디딜 때마다 전원이 처음으로 돌아가서
   20판을 끝까지 갈 수가 없다.
...
(function (global) {
  ...
  function tick(state, inputs, dt) {
    ...
    /* ---- 10a. 떨어진 사람은 자기 시작점으로 ----
       떨어지는 건 발을 헛디딘 것이라 그 사람만 물린다. 가시는 다르다(10b) —
       판이 깔아 둔 함정이고, 한 명이 걸리면 팀이 같이 물린다.
       이 둘을 같이 묶어 두면 계단에서 한 번 헛디딜 때마다 전원이 처음으로
       돌아가서, 20판을 끝까지 갈 수가 없다. */
    for (i = 0; i < keys.length; i++) {
      pid = keys[i]; p = players[pid];
      if (p.y <= L.H + FALL_OUT) continue;
