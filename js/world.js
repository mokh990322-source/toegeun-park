...
(function (global) {
  ...
  function buildStage1() {
    ...
    /* 폐기통은 가운데 섬 옆 바닥에 둔다. 벽이 아니라 바닥 위 물건이라
       ...
       col16,row9(가운데 열린 띠의 정중앙)에 두면 안 된다 — 그 자리는
       스폰에서 위쪽(ref/model)과 아래쪽(retopo/ship)을 오가는 모든
       사람이 지나는 유일한 허리 통로 한복판이고, 스폰 두 개와 60px
       밖에 안 떨어져 있다. 물건을 든 채로 그냥 지나가다 액션 키를
       누르면 조용히 버려진다. 왼쪽 섬 남서쪽 모서리, 통로에서 한 칸
       비껴난 자리로 옮긴다 — 일부러 발을 들여야 닿고, 지나가던 사람이
       실수로 밟을 일은 없다. */
    map.stations.push({ id: 'bin1', type: 'bin', cx: 9 * TILE + TILE / 2, cy: 8 * TILE + TILE / 2 });
